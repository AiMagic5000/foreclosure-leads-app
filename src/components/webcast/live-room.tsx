...
function WebcastLiveContent() {
  ...
  // Clerk signups from /webcast arrive with ?welcome=1 — fire the webcast
  // automations (email drip + confirmation + lead capture + admin notice) once,
  // then strip the flag so a refresh does not re-trigger.
  useEffect(() => {
    if (searchParams.get('welcome') !== '1') return
    fetch('/api/webcast/enroll', { method: 'POST' }).catch(() => {})
        ...
        <div className="max-w-[1400px] mx-auto flex items-center justify-between gap-3">
          ...
          <div className="flex min-w-0 flex-1 items-center gap-3 flex-wrap">
            ...
            <span className="hidden md:flex items-center gap-1 px-2 py-0.5 rounded text-xs font-bold bg-red-600 text-white">
              <Zap className="w-3 h-3" /> Limited Spots
            </span>
            <span className="hidden md:inline-flex items-center font-bold text-sm" style={{ color: BRAND.gold }}>
              Claim your Owner Operator Account &mdash; Keep 100% Commissions!
            </span>
          </div>
          ...
          <div className="flex flex-none items-center gap-3">
            ...
            {/* In the dashboard they are already signed in and in their account — hide the redundant nav button */}
            {!inDashboard && (
              <>
                <SignedIn>
                  <a
