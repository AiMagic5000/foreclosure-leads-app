...
export async function POST(request: NextRequest) {
  try {
    ...
    if (pinError) {
      return NextResponse.json(
        { error: "Failed to create account: " + pinError.message },
        { status: 500 }
      );
    }
    ...
    // Send credentials email -- wrap in try/catch so SMTP failures don't fail the provision response
    if (SMTP_PASS) {
      try {
        ...
        // Log SMTP failure but don't fail the provision -- account is already created
        console.error(
          "Provision email failed for",
          lead.email,
          ":",
